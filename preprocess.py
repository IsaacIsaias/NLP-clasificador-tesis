...
titulo = train_ds[:'Título']
carrera = train_ds[:'Carrera']
texto_procesado = preprocess(texto)
titulo_procesado = preprocess(titulo)

file = open('', 'w', encoding='ISO-8859-1')
file.write('introduccion,titulo,carrera' + '\n')
for i in range(len(texto_procesado)):
    file.write(str(texto_procesado[i]) + ',' + str(titulo_procesado[i]) + ',' + str(carrera[i]) + '\n')
file.close()
